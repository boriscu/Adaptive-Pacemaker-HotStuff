...
class Settings(BaseSettings):
    ...
    pacemaker_type: PacemakerType = Field(
        default=PacemakerType.BASELINE, description="Type of pacemaker to use."
    )
    ...
    ui_host: str = Field(
        default=DEFAULT_UI_HOST, description="Host address for the UI server."
    )
    ui_port: int = Field(
        default=DEFAULT_UI_PORT, gt=0, lt=65536, description="Port for the UI server."
    )
    ui_debug: bool = Field(
        default=DEFAULT_UI_DEBUG, description="Enable Flask debug mode."
    )

    model_config = {"env_prefix": "HOTSTUFF_", "env_file": ".env", "extra": "ignore"}
